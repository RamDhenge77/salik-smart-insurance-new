    ...
    <div className="eco-card">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {champions.map((champion, index) => (
          <div
            key={index}
            className="bg-eco-gray-light rounded-lg p-4 transition-all hover:shadow-md"
          >
            ...
            <div className="border-t border-eco-gray pt-3 mb-3">
              <div className="flex items-center mb-1">
                <Avatar className="h-10 w-10 mr-3 border">
                  {champion.warrior.avatar.includes("/lovable-uploads") ? (
                    <img
                      src={champion.warrior.avatar}
                      alt={champion.warrior.name}
                      className="p-2"
                    />
                  ) : (
                    <>
                      <AvatarImage
                        src={champion.warrior.avatarImg}
                        alt={champion.warrior.name}
                      />
                      <AvatarFallback>{champion.warrior.avatar}</AvatarFallback>
                    </>
                  )}
                </Avatar>
            ...
            {champion.reward && (
              <div className="bg-gradient-to-br from-btn-primary to-themeDark rounded-lg p-3 text-white mt-2">
                <Badge
                  variant="outline"
                  className="bg-white bg-opacity-20 text-white mb-1 border-none"
                >
                  {champion.reward.title}
                </Badge>
                <p className="text-sm font-semibold mb-2">
                  {champion.reward.description}
                </p>
                ...
                {champion.reward.validUntil && (
                  <p className="text-xs text-white text-opacity-80">
                    Valid until: {champion.reward.validUntil}
                  </p>
                )}
              </div>
            )}
